...
import { getAuthor, socialShare, getTags, getCategories } from '../../../helper/helper';
                    ...
                    <div className="row">
                        <div className="col-md-8">
                            ...
                            <div className="post_meta">
                                <ul>
                                    <li><i className="fa fa-user" /> Post by:
                                        {getAuthor(item.author).map((author, i) => (
                                            <Link to={"/blog/author/" + author.id} className="meta_admin" key={i}>{author.name}</Link>
                                        ))}
                                    </li>
                                    <li><i className="fas fa-tags" />
                                        {/* Data */}
                                        {getCategories(item.category).slice(0, 3).map((cat, i) => (
                                            <span key={i}> <Link to={"/blog/cat/" + cat.id}> {cat.title} </Link></span>
                                        ))}
                                        {/* Data */}
                                    </li>
                                </ul>
